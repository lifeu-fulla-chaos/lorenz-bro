...
# Slave Lorenz system with ASMC control
function slave_lorenz_asmc!(du, u, p, t)
    x1, x2, x3 = u[1:3]  # Slave states
    xm1, xm2, xm3 = u[4:6]  # Master states
    u_control = p[1](u, t)  # ASMC control input
...
function energy_consumption(sol)
    ...
    # Use quadgk to integrate the squared control signals
    energy = quadgk(t -> sum([u_control_squared[j] for j in 1:3]), sol.t[1], sol.t[end])[1]
    return energy
end
...
# Plot results
p1 = plot(sol_adaptive, vars=(1, 4), label=["Slave x1 (Adaptive)" "Master x1"], title="Lorenz Synchronization Adaptive Control", xlabel="Time", ylabel="x1")
p2 = plot(sol_asmc, vars=(1, 4), label=["Slave x1 (ASMC)" "Master x1"], title="Lorenz Synchronization ASMC", xlabel="Time", ylabel="x1")


savefig(p1, "Lorenz_Synchronization_Adaptive_Control.png")
savefig(p2, "Lorenz_Synchronization_ASMC.png")
