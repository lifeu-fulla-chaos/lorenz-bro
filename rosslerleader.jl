...
function drive_system(x, p, t)
    # Example: rossler system or another chaotic system
    d, e, f = p[4:6]
    x1, x2, x3 = x
    dx1 = -x[2] - x[3]
    dx2 = x[1] +(d * x[2])
...
function backstepping_control(x, y, p, k)
    ...
    u1 = -x[2] - x[3] - (σ * (y[2] - y[1])) + e2
    u2= x[1] + (d * x[2])-(ρ * y[1]) + y[2] + (y[1] * y[3]) + e3
    u3= e + (x[3] * (x[1] - f)) - (y[1] * y[2]) + (β * y[3]) - ((3 + (2 * k[1])) * e1) - ((5 + ( 2 * k[1])) * e2) - ((3 + k[1]) * e3)
...
# Save plots
savefig(pl1, "synchronization_x1_y1_ross_lor.png")
savefig(pl2, "synchronization_x2_y2_ross_lor.png")
savefig(pl3, "synchronization_x3_y3_ross_lor.png")
...
# Save plot
savefig(p_error, "synchronization_errors_ross_lor.png")
